...
def home_with_events(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    ...
    store = Store(config)
    store.append_event(session_id="abc", platform="claude", cwd="/x",
                       t="user_message", data={"prompt": "hello"})
    store.append_event(session_id="abc", platform="claude", cwd="/x",
                       t="tool_call",
                       data={"tool_name": "Edit",
                             "tool_input": {"old_string": "x"}})
    sd = session_dir(tmp_path, "claude", "abc")
    EvalStore(sd).append(EvalResult(
        id="01J", session_id="abc", definition="default", agent="claude",
...
def test_events_no_findings_flag_hides_them(home_with_events: Path):
    result = CliRunner().invoke(events_cmd, ["abc", "--no-findings"],
...
def test_events_eval_filter(home_with_events: Path):
    ...
    EvalStore(sd).append(EvalResult(
        id="01K", session_id="abc", definition="token-efficiency",
        agent="claude", agent_model="", agent_session_id=None,
        started_at="t", ended_at="t", duration_ms=0,
        verdict="pass", summary="",
        findings=[Finding(seq=1, severity="info", note="cache hit")],
    ))
    result = CliRunner().invoke(events_cmd, ["abc", "--eval", "default"],
                                catch_exceptions=False)
    assert result.exit_code == 0
    assert "redundant edit" in result.output
